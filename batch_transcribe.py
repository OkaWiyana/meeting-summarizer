...
batch_transcribe.py
===================
Skrip batch untuk transkripsi otomatis semua file audio rapat:
  - Scan folder: dataset/01_raw/video_mp4/, audio_mp3/, audio_wav/
  - MP4 → ekstrak audio ke WAV dulu, lalu transkripsi
...
# ── Pastikan import dari modules/ bisa diakses ───────────────
...
# Untuk TRAINING  : ['wav']  ← default, 1 transkripsi per sesi
# Untuk EVALUASI  : ['mp4', 'mp3', 'wav']  ← notebook 03 (timing comparison)
...
def kumpulkan_file_audio() -> list[tuple[Path, str]]:
    """
    Kumpulkan file audio dari folder raw sesuai FORMAT_FILTER.

    Return:
        List of (path_file, format_label) — hanya format yang ada di FORMAT_FILTER.
    """
    file_list = []
...
def transkripsi_satu_file(audio_path: Path, fmt: str) -> tuple[str, float]:
    """
    Transkripsi satu file audio dan kembalikan teks + waktu proses.

    Parameter:
        audio_path : Path ke file audio.
        fmt        : Format file ('mp4', 'mp3', 'wav').

    Return:
        Tuple (teks_transkripsi, waktu_detik)
    """
    target_path = audio_path

    # Ekstrak audio dari video MP4 terlebih dahulu
    if fmt == "mp4":
...
def main() -> None:
    ...
    if not file_list:
        print("\n⚠️  Tidak ada file audio ditemukan di folder raw.")
